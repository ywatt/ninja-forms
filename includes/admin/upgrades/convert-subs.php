...
class NF_Convert_Subs {
	...
	public function get_old_subs( $begin = '', $count = '' ) {
		...
		$subs_results = $wpdb->get_results( 'SELECT * FROM ' . NINJA_FORMS_SUBS_TABLE_NAME . $limit, ARRAY_A );
	...
	public function count_old_subs() {
		global $wpdb;
		$count = $wpdb->get_results( 'SELECT COUNT(*) FROM '. NINJA_FORMS_SUBS_TABLE_NAME, ARRAY_A );
